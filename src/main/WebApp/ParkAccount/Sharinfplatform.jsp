 				...
 				<blockquote class="layui-elem-quote">
					<div class="demoTable">
  						...
  						<div class="layui-inline">
		    				<input class="layui-input" id="parkingSpace_parkingSpaceNumber_query" autocomplete="off">
  						</div>
  						</div>
				</blockquote>
  			</div>
...
layui.use(['table','laypage'], function(){
  ...
  table.render({
    elem: '#table'
    ,height: 'full-300'
    ,url: '/sharingplatform/getallspbypage'
    ,where:{
			currentPageNo:currentPageNo, 
			pageSize:pageSize, 
        	'parkingSpace.parkingSpaceNumber': $('#parkingSpace_parkingSpaceNumber_query').val()
		} //传参*/  
...
//     ,page: {theme: '#c00',  limits:[5,10,25,50], groups:5, limit:25/* , curr:7 */, id: 'abc1'}
    ...
    ,cols: [[
        ...
        ,{field:'parkingSpaceId',templet:'#tpl_parkingSpace_parkingSpaceNumber',title: '停车位编号', sort: true}
        ,{field:'sharingStatusId',templet:'#tpl_sharingStatus_statuSname', title: '车位共享状态'}
        ,{field:'sharerId',templet:'#tpl_sharer_nickName', title: '共享者车主名称', sort: true}
        ,{field:'userId', templet:'#tpl_user_userName', title: '使用者车主名称', sort: true}
        ,{field:'parkingId', templet:'#tpl_parkingInfo_parkingName',title: '停车场编号'}
		...
		if(layEvent === 'detail'){ //查看
			layerOut(layEvent, data.id);
		 	 ...
		 	 layer.confirm('真的删除行么', function(index){
				//向服务端发送删除指令
				$.post('/sharingplatform/delsharingplatform', {id:data.id}, function(data){
					var result;
...
function layerOut(eventType, id, obj){//obj是传进来，给修改成功后：要更新的行Dom对象
	...
	layui.use(['layer', 'form'], function(){
		var $ = layui.$;
	 	...
	 	$.get('/sharingplatform/layeroptions', {eventType:eventType, id:id}, function(form) {
	 		layer.open({
				type: 1,
				title: '停车位信息',
				content: form,
...
// 						这里可以写ajax方法提交表单
						if(eventType=="add"){
	 						$.post("/sharingplatform/addsharingplatform", data.field, function(data){
	 							var result;
	 						...
	 						$.post("/sharingplatform/updatesharingplatform", data.field, function(data1){
	 							var result;
	 							(data1.result == "true") ? result="修改成功": result="修改失败";
	 	 						...
	 	 						//进行局部修改
	 	 						$.post("/sharingplatform/getsharingplatformbyid", data.field, function(data2){
	 	 							$(obj.tr).find("td div:eq(5)").html("<a style='color:red'>"+data2.sharingStatus.statuSname+"</a>");
	 	 						})
	 	 						obj.update({
	 	 							title: data.sharingPlatformUpdate.parkingSpaceId
		 	 			 		});
	 	 						/* //进行局部修改
...
<!-- 复杂表头属性绑定 -->
 ...
 <script type="text/html" id="tpl_parkingSpace_parkingSpaceNumber">
	{{# if(d.parkingSpace != null){ }}
		{{d.parkingSpace.parkingSpaceNumber}}
	{{# } }}
</script>

 <script type="text/html" id="tpl_sharingStatus_statuSname">
	{{# if(d.sharingStatus != null){ }}
		{{d.sharingStatus.statuSname}}
	{{# } }}
</script>

 <script type="text/html" id="tpl_sharer_realName">
	{{# if(d.sharer != null){ }}
		{{d.sharer.realName}}
	{{# } }}
</script>

<script type="text/html" id="tpl_user_userName">
	{{# if(d.user != null){ }}
		{{d.user.userName}}
	{{# } }}
</script>
